...
//    final static Integer month = 8;
    ...
    static void writeBlockCellRange(XSSFSheet sheet, int LeftOffset, int TopOffset, HashMap<String, HashMap<Integer, Double>> data, String name) {
    ...
    private static void processAllCount(XSSFSheet sheet, int leftOffset, int topOffset, HashMap<String, HashMap<Integer, Double>> data, Integer end, Integer begin, String name) {
        ...
        XSSFCell cell20 = currentRow2.createCell(leftOffset);
        ...
        XSSFCell cell30 = currentRow3.createCell(leftOffset);
    ...
    private static void setDate(XSSFSheet sheet, Integer start, Integer end, int leftOffset, int topOffset) {
        ...
        for (int i = start; i <= end; i++) {
            ...
            XSSFCell cell = currentRow.createCell(leftOffset);
    ...
    private static XSSFCellStyle getCellStyle(XSSFSheet sheet) {
        ...
        font.setColor(new XSSFColor(new java.awt.Color(18,52,86)));
    ...
    private static void processTitle(XSSFSheet sheet, int LeftOffset, int TopOffset, String name) {
        CellRangeAddress cellAddresses = new CellRangeAddress(TopOffset, 1 + TopOffset, LeftOffset, 3 + LeftOffset);
        XSSFCellStyle cellStyle = getCellStyle(sheet);

        sheet.addMergedRegion(cellAddresses);
        XSSFRow row0 = sheet.getRow(TopOffset);
        if (row0 == null) {
            row0 = sheet.createRow(TopOffset);
        }
        XSSFRow row1 = sheet.getRow(1 + TopOffset);
        if (row1 == null) {
            row1 = sheet.createRow(1 + TopOffset);
        }
        XSSFCell cell00 = row0.createCell(LeftOffset);
